...
type ConfigMonitor struct {
	...
	configManager     *configuration.ConfigCache
...
// GetInstance returns an singleton instance of ConfigMonitor
// or an error if fsnotify fails to initialize
func GetInstance(ctx context.Context) (*ConfigMonitor, error) {
	if monitor == nil {
		...
		configManager := configuration.GetCacheInstance()
...
func (cm *ConfigMonitor) TrackNew(path string, config interface{}) error {
	c, err := configuration.NewConfigurationFile(path, config)
	if err != nil {
		return err
	}
